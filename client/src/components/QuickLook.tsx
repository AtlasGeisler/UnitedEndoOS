...
export function QuickLookProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      else if (e.key === "+" || e.key === "=") setT((p) => ({ ...p, zoom: Math.min(6, Math.round((p.zoom + 0.1) * 10) / 10) }));
      else if (e.key === "-") setT((p) => ({ ...p, zoom: Math.max(0.1, Math.round((p.zoom - 0.1) * 10) / 10) }));
    };
        ...
        {items && current && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <div className="relative flex flex-1 items-center justify-center overflow-hidden"
              ...
              onWheel={(e) => setT((p) => ({ ...p, zoom: Math.min(6, Math.max(0.1, Math.round((p.zoom - e.deltaY * 0.002) * 10) / 10)) }))}
            ...
            <div className="flex flex-wrap items-center justify-center gap-4 px-4 py-3 text-white">
              <Tool icon={ZoomOut} onClick={() => setT((p) => ({ ...p, zoom: Math.max(0.1, Math.round((p.zoom - 0.1) * 10) / 10) }))} />
              <span className="w-10 text-center text-[12px] tnum">{Math.round(t.zoom * 100)}%</span>
              <Tool icon={ZoomIn} onClick={() => setT((p) => ({ ...p, zoom: Math.min(6, Math.round((p.zoom + 0.1) * 10) / 10) }))} />
